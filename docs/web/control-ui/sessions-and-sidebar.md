...
The sidebar organizes everything around the agent; the session list includes pinned destinations above ordinary zones. Ordinary child rows stay outside root grouping, pinning, dragging, multi-select, and pagination; dashboard children remain eligible for global pinning as user-facing destinations. Collapsed zones do not consume the visible page budget. The remaining session, activity, grouping, and menu behavior is unchanged.
